...
Plot_formulae function is to compare with the plots generated by Cary Colgan in his PhD thesis.
...
def dc_bw(root_s: np.ndarray[float], theta: np.ndarray[float]):
...
def c_bw(root_s: np.ndarray[float]):
