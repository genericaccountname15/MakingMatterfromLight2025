"""
runs the Germanium lambertian simulation while changing the angle
"""
import numpy as np
...
run_data_collection(
    variables = np.linspace(0, 90, 10)[7:] * np.pi / 180,
    variable_name = 'angle',
    variable_parameter_name = 'rotation',
    units = 'radians',
    old_value = values.source_angle * np.pi / 180,
    xray_type = 'lambertian_Ge',
    repeat = 3,
    additional_label = 'Ge_4Aug_part2',
    sim_params = accurate,
    sample_params = deep
    )
