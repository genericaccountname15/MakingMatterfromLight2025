...
Introduce a parameter f which labels the distance of the tungsten
shielding from the centre of the beam
...
class Hit_counter(Simulation):
    ...
    """
    def __init__(self, xray_bath, gamma_pulse, f, n_samples_angular=400, n_samples=10, n_samples_azimuthal = 1):
    ...
    def calc_effective_height(self, r, phi, d, f):
        dist = r + f

        d_prime = d - f # new effective d even for 0 deg case
        #d now defines perpendicular distance to circle centre
    ...
    def calc_effective_d(self, r, phi, d, f):
        d_prime = d - f
        m = abs( np.tan(phi) )
        x2 = ( d - np.sqrt( d * d 
                                - ( 1 + m * m) * ( d * d - r * r ))
